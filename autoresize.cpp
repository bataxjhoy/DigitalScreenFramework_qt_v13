...
void AutoResize::dealLayoutItems_xjh(QWidget* obj)
{
    // 输出当前控件的信息
    qDebug() <<"find "<< obj;
    // 遍历当前控件的所有直接子控件
    ...
    for (QObject *child : children) {
        // 如果子控件是 QWidget 或其子类，则递归遍历
        if (QWidget *childWidget = qobject_cast<QWidget *>(child)) {
//             qDebug() <<"childWidget" <<childWidget;
//              qDebug() <<"childWidget->parent()" <<childWidget->parent();
            m_otherItem.push_back(qobject_cast<QWidget *>(child->parent()));
            dealLayoutItems_xjh(childWidget);
        }else{
            m_otherItem.push_back(qobject_cast<QWidget *>(child->parent()));
            ignoreAllChiledren(qobject_cast<QWidget *>(child->parent()));
        }
    }
}
